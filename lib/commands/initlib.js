        ...
        if (os.type() + '' === 'Linux'){
          ...
          if (os.type() + '' === 'Linux'){
            ...
            setTimeout(() =>{
              ...
              // Change the mode from spa to universal if Universal mode is preferred.
              if (mutate) {
                let configFile = fs.readFile(`./${dir}/nuxt.config.js`, 'utf8', err => {
                   if (err)
                    throw err;
                }).toString().split('\n');
                for (let index = 0; index < configFile.length; index++) {
                  if (configFile[inde].includes('mode')) {
